...
export function drawHexagon(ctx, shape, roughOps = null) {
    ...
    const pathString = `M ${-hw + offset} ${-hh} L ${hw - offset} ${-hh} L ${hw} 0 L ${hw - offset} ${hh} L ${-hw + offset} ${hh} L ${-hw} 0 Z`;
    ...
    ctx.beginPath();
    ctx.moveTo(-hw + offset, -hh);
    ctx.lineTo(hw - offset, -hh);
    ctx.lineTo(hw, 0);
    ctx.lineTo(hw - offset, hh);
    ctx.lineTo(-hw + offset, hh);
    ctx.lineTo(-hw, 0);
