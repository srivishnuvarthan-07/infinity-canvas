            ...
            <ScrollArea className="flex-1">
                <div className="p-4 grid grid-cols-2 gap-3">
                    ...
                    {sortedItems.map(item => (
                        <div
                            key={item.id}
                            className="group relative aspect-square bg-neutral-800/50 rounded-lg border border-neutral-800 hover:border-neutral-600 transition-colors flex flex-col items-center justify-center cursor-grab active:cursor-grabbing p-2"
                            draggable
                            onDragStart={(e) => handleDragStart(e, item)}
                        >
                            {/* Preview Placeholder (Real preview would need canvas rendering) */}
                            <Box className="w-8 h-8 text-neutral-600 mb-2 group-hover:text-blue-500 transition-colors" />

                            <span className="text-[10px] text-neutral-400 font-medium truncate w-full text-center px-1">
                                {item.name}
                            </span>
                            ...
                            <Button
                                variant="ghost"
                                size="icon"
                                className="absolute top-1 right-1 h-6 w-6 opacity-0 group-hover:opacity-100 hover:bg-red-500/20 hover:text-red-400 transition-all"
                                onClick={(e) => {
                                    e.stopPropagation();
